...
description: "Build bots, indexers, and frontends on maki.finance with @vetostudio/veto-sdk: typed reads and writes, exact quote math, and event streaming."
icon: "js"
---

`@vetostudio/veto-sdk` is the official TypeScript SDK for the maki.finance protocol. It's a thin, fully-typed layer over [viem](https://viem.sh) (its only peer dependency) covering the entire protocol surface: launching tokens, trading the curve, quoting, migration, fee claims, and event streaming.

```bash
npm install @vetostudio/veto-sdk viem
```
...
import { createVeto, robinhoodMainnet } from "@vetostudio/veto-sdk";
...
- `veto.read`: typed views over the launchpad (curve state, quotes, fees, registry, pool info)
...
const states = await veto.read.getCurveStates(tokens);

// Per-token views
const info = await veto.read.getTokenInfo(tokens[0]);
// → { address, name, symbol, metadataURI }
const state = await veto.read.getCurveState(tokens[0]);
...
```ts
import { quoteBuy, quoteSell, currentPrice, curveProgressBps, ethToGraduate, totalFeeBpsOf } from "@vetostudio/veto-sdk";

const state = await veto.read.getCurveState(token);
...
const onchain = await veto.read.quoteBuy(token, parseEther("1"));
```
...
import { vetoDotFunAbi } from "@vetostudio/veto-sdk";

const hash = await veto.write.createToken({
  name: "My Star",
  symbol: "MSTAR",
...
<Note>
  Every `veto.write.*` method **simulates first** (so reverts are decoded before you spend gas) and returns the transaction `Hash`. Await the receipt yourself with `publicClient.waitForTransactionReceipt`.
</Note>
...
const q = quoteBuy(await veto.read.getCurveState(token), totalFeeBps, ethIn);

// Exact ETH in; slippage (default 1%) and deadline (default 5 min) built in
const hash = await veto.write.buy(token, ethIn, q.tokensOut, { slippageBps: 100 });
await publicClient.waitForTransactionReceipt({ hash });

// Or exact tokens out, refunding any excess ETH
await veto.write.buyExactTokens(token, tokensWanted, maxEthIn);
```
...
```ts
const balance = await veto.read.balanceOf(token, account.address);
const sq = quoteSell(await veto.read.getCurveState(token), totalFeeBps, balance / 2n);

// One transaction: signs a permit, no separate approve needed
await veto.write.sellWithPermit(token, balance / 2n, sq.ethOut);

// Classic two-step path also available:
await veto.write.approve(token, amount);
await veto.write.sell(token, amount, sq.ethOut, { slippageBps: 50 });
```
...
await publicClient.waitForTransactionReceipt({ hash: await veto.write.migrate(token) });

// Inspect the resulting Uniswap v3 pool
const pool = await veto.read.getPool(token);              // zero address until pool exists
const { sqrtPriceX96, liquidity } = await veto.read.getPoolState(pool);
const positionId = await veto.read.positionOf(token);     // locked LP NFT id (0 = not migrated)
```
...
const owed = await veto.read.creatorFeesOwed(token);

// Fee recipient or claim authority only; pays the recipient
await veto.write.claimCreatorFees(token);

// Airdrop mode (claim authority): pay many tokens' recipients in one tx
await veto.write.claimCreatorFeesBatch([tokenA, tokenB, tokenC]);
...
const locker = await veto.read.locker();
await veto.write.collectFees(locker, token);
...
```ts
const lastBlock = await veto.backfill({
...
<AccordionGroup>
  ...
  <Accordion title="ABIs">
    `vetoDotFunAbi`, `vetoTokenAbi`, `feeLockerAbi`, exported for direct viem/wagmi use and event parsing.
  </Accordion>
</AccordionGroup>
