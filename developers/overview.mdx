...
2. creates and initializes the USDC/token 1% pool at the global opening price,
...
<Warning>
  The Arc deployment is in progress. Addresses will appear here and in `@vetodotfun/sdk`'s `deployments.arc` the moment they are live and source-verified; until then the SDK's built-in network entries are guarded placeholders that throw a descriptive error.
</Warning>
...
<Note>
  Uniswap v3 is not deployed on Arc testnet, so neither are the Veto contracts. Develop against an `arc-anvil` fork of mainnet ([Arc Foundry](https://docs.arc.io/arc/references/evm-differences)) and pass the fork's deployment object to the SDK.
</Note>
