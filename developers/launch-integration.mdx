...
<Steps>
  ...
  <Step title="Route the creator fee stream (optional)">
    By default the creator's share of pool fees is paid to the launching wallet. To pay a different wallet — a team multisig, a partner, your own platform — redirect it right after launch:

    ```ts
    await veto.write.setFeeRecipient(token, "0xRecipient…");
    ```

    Only the current recipient can call this, and it moves the **whole** creator share. veto.fun tracks `FeeRecipientSet` and shows the recipient wallet the token in its studio, so whoever you route to can claim without any help from you.

    <Note>
      A `feeRecipient` parameter on `createToken` itself (one transaction) ships with the Arc contract generation; until then it is this second call.
    </Note>
  </Step>
